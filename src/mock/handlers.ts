...
function withEmployee(beacon: Beacon) {
  return {
    ...beacon,
    employee: mockEmployees.find((employee) => idsMatch(employee.id, beacon.employee_id)) ?? null,
  };
}
  ...
  http.get(apiPath(API_ENDPOINTS.attendance.today), () => {
    ...
    const employees: TodayAttendanceStatus[] = mockEmployees.map((employee) => {
      ...
      return {
        employee,
        attendance,
        status: attendance === null ? "not_checked_in" : attendance.check_out ? "checked_out" : "checked_in",
      };
    });
